...
- 自动分类：按关键词归类为时政、国际、科技、财经、体育、娱乐、综合，并标记境内、境外。
...
python -m backend.topnews_backend.cli scheduler --news-interval-minutes 10 --papers-interval-minutes 60 --figures-interval-minutes 180
...
- `GET /health`
- `POST /v1/ingest`
- `GET /v1/news?page=1&pageSize=20&category=时政&region=境内`
...
- `scheduler`：常驻进程，默认启动时立即抓一次，之后每 10 分钟抓新闻、每 60 分钟抓论文；论文抓取完成后会立即补一轮主图，独立主图兜底任务每 180 分钟扫描缺图论文。
...
Environment=TOPNEWS_CONFIG=/opt/topnews/backend/sources.example.json
Environment=TOPNEWS_DB=/opt/topnews/backend/data/topnews.db
ExecStart=/opt/topnews/.venv/bin/python -m backend.topnews_backend.cli scheduler --news-interval-minutes 10 --papers-interval-minutes 60 --figures-interval-minutes 180 --paper-source auto
Restart=always
RestartSec=10
